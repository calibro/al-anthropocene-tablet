app.controller('PlayCtrl', function($scope,$rootScope,$state,socket,$timeout,playlistService,$ionicHistory,$ionicScrollDelegate,Idle) {
    ...
    socket.emit('changeView',{view:"play",room:room});

    $scope.chunks = playlistService.getPlaylist();
    socket.emit('playlist',{chunks: $scope.chunks, room:room});
  ...
  $scope.selectChunk = function(id,ind) {
    ...
    socket.emit('playChunk',{chunk:id, room:room});
  };
  ...
  $scope.playOrPause = function() {
    ...
    if($scope.play){
      socket.emit('playStatus',{status:"play",room:room});
    }
    else {
      socket.emit('playStatus',{status:"pause",room:room});
    }
  }
    ...
    $scope.go = function(where) {
        $state.go(where);
        //Idle.watch();
      socket.emit('changeView',{view:"create",reload:false,room:room});
    }
...
*/
    ...
    $scope.reset = function() {
      socket.emit('changeView',{view:"create",reload:true,room:room});
      $ionicHistory.clearCache();
